...
export const databaseService = {
  ...
  async addProduct(productData: AddProductForm): Promise<{
    success: boolean
  ...
  async getProductsByCategory(categoria: string): Promise<{
    success: boolean
    products?: DatabaseProduct[]
    error?: DatabaseError
  }> {
  ...
  async createOrder(
    ...
    try {
      ...
      if (pedidoError) {
        return {
          success: false,
          error: handleDatabaseError(pedidoError, "createOrder - pedido insert"),
        }
      }
      ...
      const { error: detallesError } = await supabase.from("pedido_detalles").insert(detallesData)
      ...
      if (detallesError) {
        ...
        return {
          success: false,
          error: handleDatabaseError(detallesError, "createOrder - detalles insert"),
        }
      }
  ...
  async getOrderById(pedidoId: string): Promise<{
    success: boolean
  ...
  async getOrdersByUser(usuarioId: string): Promise<{
    success: boolean
    pedidos?: PedidoCompleto[]
    error?: DatabaseError
  }> {
  ...
  async getAllUsers(): Promise<{
    ...
    try {
      ...
      if (error) {
        return {
          success: false,
          error: handleDatabaseError(error, "getAllUsers"),
        }
      }
